...
5. Add include guards to the header
   #+begin_src shell
     ...
     EOF
   #+end_src
6. In =test/test-suite.c= add the appropriate include header.
7. Start adding your tests (see [[Adding more Tests to Ones that Exist][Adding more Tests to Ones that Exist]])
