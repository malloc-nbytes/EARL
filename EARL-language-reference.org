...
take a slice of the list and create a new list of those elements.

A =slice= is defined as:

Let $S$ be a "starting" <expr>, $E$ be an "ending" <expr>, and L be some nonempty list of elements
where $S \le E$ in
...
Another important note is that the underlying value that the =option= holds are dynamic:
