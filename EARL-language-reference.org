...
functions, and classes. You can have none, one, or multiple.

| Name     | Variables | Functions | Classes | Description                                                  |
|----------+-----------+-----------+---------+--------------------------------------------------------------|
| =@pub=   | YES       | YES       | YES     | Makes the identifier available to other files                |
| =@world= | NO        | YES       | NO      | The function closes in the world scope                       |
| =@ref=   | YES       | NO*       | NO      | Declares the variable to be a reference (see the note below) |
...
#+begin_quote
Note: These member intrinsics for =str= is currently
a work in progress and is not guaranteed to work.
#+end_quote
