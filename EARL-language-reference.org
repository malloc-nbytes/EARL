...
| to \rightarrow [[For Loops]], [[Foreach Loops]] |
...
#+begin_quote
Below is a table of all valid binary operators.
#+end_quote
...
| ///      | Division                      |
| =%=      | Modulo                        |
| =`&=     | Bitwise AND (backtick needed) |
| =`~=     | Bitwise NOT (backtick needed) |
| =`^=     | Bitwise XOR (backtick needed) |
...
=class= <id> ?(=[= <arg1>, <arg2>,..., <argN> =]=) ={=
    ...
    =@= *(pub | world | ref) let <member1> = <id> | /expr/ =;=

    =@= *(pub | world | ref) let <member2> = <id> | /expr/ =;=
...
If the value _does_ exist, it will return it in a wrapped =option= value.
