...
*Note*: You can also use Emacs keybindings such as =C-f=, =C-b=, =C-p=, =C-n=, =M-f=, =M-b=, etc.
...
| @const \rightarrow [[Attributes]]                              |
| @experimental \rightarrow [[Attributes]]                       |
| return                                                     |
| true                                                       |
| false                                                      |
| none                                                       |
...
#+begin_quote
You can use BASH commands directly in EARL source code, and
there are three different ways to do this:
1. by just having the embed bash.
2. by piping the output into a new variable
3. by piping the output into an existing variable/location.

*Note*: For convenience, if a bash command returns a string of size 1
that is a space, newline, tab etc, EARL will cut those out. This can be disabled
using the =--no-sanitize-pipes= flag.
#+end_quote
...
| =@experimental= | YES       | YES       | YES           | YES           | Shows an `experimental` warning once upon use                |
