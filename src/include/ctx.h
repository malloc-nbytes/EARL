...
struct ctx {
        ...
        struct s_umap(const char *, struct stmt_fn *) *fscope;
...
struct ctx ctx_create(unsigned long (*scope_hash)(const char *),
...
int ctx_var_in_scope(struct ctx *ctx, const char *id);

/// @brief Assert that a variable is in scope
/// @note Will panic if it does not exist
/// @param ctx The context
/// @param id The ID
void ctx_assert_var_in_scope(struct ctx *ctx, const char *id);

/// @brief Assert that a variable is not in scope
/// @note Will panic if it does exist
/// @param ctx The context
/// @param id The ID
void ctx_assert_var_not_in_scope(struct ctx *ctx, const char *id);

/// @brief Push the variable scope
/// @param ctx The context
void ctx_push_scope(struct ctx *ctx);

/// @brief Push the function scope
/// @param ctx The context
void ctx_push_fscope(struct ctx *ctx);

void ctx_add_var_to_scope(struct ctx *ctx, const char *id);
