...
printCtxTbl (Ctx tbl) = mapM_ printMap tbl
  where
    ...
    printVar (token, var) = putStrLn $ "Token: " ++ show token ++ ", Variable: " ++ show var
...
evaluateExpr expr ctx =
  case expr of
    ...
    ExprTerm (TermIntlit tok) -> undefined
...
evaluateLetStmt :: LetStmt -> Ctx a -> Ctx a
...
evaluateStmt :: Stmt -> Ctx a -> IO (Ctx a)
