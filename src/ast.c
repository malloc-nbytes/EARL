...
static void dump_stmt(const struct stmt *stmt, int pad);
...
static void
dump_expr_term_identifier(const struct expr_identifier *id, int pad) {
        (void)pad;
        printf("%s", id->identifier->lx);
}

static void
dump_expr_term_integer_literal(const struct expr_integer_literal *integer, int pad) {
        (void)pad;
        printf("%s", integer->integer->lx);
}

static void
dump_expr_term_string_literal(const struct expr_string_literal *str, int pad) {
        (void)pad;
        printf("\"%s\"", str->string->lx);
}

static void
dump_expr_term_character_literal(const struct expr_character_literal *chara, int pad) {
        (void)pad;
        printf("'%s'", chara->character->lx);
}

static void
dump_expr_term_function_call(const struct expr_function_call *expr, int pad) {
...
static void
dump_expr_term(const struct expr_term *expr, int pad) {
        switch (expr->type) {
...
static void
dump_expr_binary(struct expr_binary *bin, int pad) {
        printf("(");
...
void
dump_expr(const struct expr *expr, int pad) {
        switch (expr->type) {
...
static void
dump_stmt_let(const struct stmt_let *let, int pad) {
        printf("LET %s = ", let->identifier->lx);
        dump_expr(let->expr, pad);
}

static void
dump_stmt_expr(const struct stmt_expr *stmt, int pad) {
        printf("(stmt_expr) ");
        dump_expr(stmt->expr, pad);
}

static void
dump_stmt_block(const struct stmt_block *stmt, int pad) {
        for (size_t i = 0; i < stmt->stmts_len; ++i)
                dump_stmt(stmt->stmts[i], pad+1);
}

static void
dump_stmt_fn(const struct stmt_fn *stmt, int pad) {
...
static void
dump_stmt_return(struct stmt_return *stmt, int pad) {
        printf("RETURN ");
        dump_expr(stmt->expr, pad);
}

static void
dump_stmt(const struct stmt *stmt, int pad) {
        padding(pad);
        switch (stmt->type) {
        case STMT_TYPE_LET: {
...
void
ast_dump(const struct program *prog) {
...
const char *
stmt_type_to_cstr(enum stmt_type type) {
        switch (type) {
